...
object PoeRpcs {
      ...
      case Some(qi) =>
        ...
        get[Any](qi.url, qi.params, qi.requestType).onComplete {
          ...
          case Failure(UnauthorizedAccessFailure(msg)) =>
            ...
            Alerter.error(s"""Unauthorized access to ${Alerter.featuresLink("https://www.pathofexile.com")}, please log in there first.""")
  ...
  private case class QueueItem(url: String, params: js.Any, requestType: AjaxHelp.HttpRequestTypes.HttpRequestType) {
    ...
    def debugLog(msg: String) = {
      if (debuggMode) console.debug(msg, id, promise.isCompleted, url, params)
    }
  ...
  // if we hit 45 req in 12 sec (which for me - looty makes 4req/s ) than we wait next 48s
